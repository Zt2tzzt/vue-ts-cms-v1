...
- 方法三：拿到菜单后，动态添加路由，通常有两种方式。
  1.  后端返回数据中有 `component` 字段，里面是组件名称，比如 `Role.vue`
...
在 `MainHeader.vue` 中发送事件，传递图标点击后的状态；在 `Main.vue` 中监听事件；做四件事：

- `MainHeader.vue` 中图标的切换（同样也是用动态组件 `<Component>`，展示图标，方便图标的切换）。
- `MainMenu.vue` 中将菜单折叠，改变 `<el-menu>` 的 `collapse` 属性。
- `MainMenu.vue` 中顶部 `h2.title` 的文本隐藏。
- `Main.vue` 中，改变 `<el-aside>` 宽度。
