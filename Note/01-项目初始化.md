...
```json
{
  ...
  "compilerOptions": {
    "baseUrl": ".",
    // 有利于编辑工具更好的读取路径，给出提示（实际打包使用的是 vite.config.ts 中的配置，如果使用 tsc 编译，会用到这里的配置）
    "paths": {
      "@/*": ["./src/*"]
    }
  },
...
> `tsconfig.json` 推荐进行项目开发相关的配置。
>
> `tsconfig.config.json` 推荐在其中进行项目打包相关的配置。
>
