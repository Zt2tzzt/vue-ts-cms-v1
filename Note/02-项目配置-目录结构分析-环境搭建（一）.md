...
>
> 如果想要在编辑器（如 VSCode）中（保存文件时）使用 prettier 格式化，需要安装 _Prettier - Code formatter_ 插件。并在 VSCode 中进行设置：
>
...
使用 git Husky 配置项目代码提交的规范。配置的两个前提：
...
git commit 前，校验 eslint，如果校验不通过，则组织提交。

:three: git husky 是什么？

*git husky* 是一个 git hook 工具，可以在 git commit 的各个阶段触发：
...
2.安装 *cz-conventional-changelog*，并且初始它，[参考文档](https://github.com/commitizen/cz-cli/issues/858)
...
执行完陈上面的命令后，`package.json` 中添加了配置如下：
...
用于限制代码的提交形式，使得代码必须按照 commitizen 的规范来提交。
...
- 避免 eslint 的 [no-undef](https://eslint.org/docs/rules/no-undef) 检测，添加一行注释，表示该文件运行在 node 环境下。
...
通过手动注释代码的形式，区分生产环境和开发环境中的网络请求地址等信息。
...
> 使用 `preview` 命令（需要先对项目进行打包），预览打包后项目的运行效果，此时处于生产环境中。
